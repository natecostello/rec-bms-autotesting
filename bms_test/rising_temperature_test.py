...
class RisingTemperatureTest(unittest.TestCase):
    """"This test case verifies response and protections to rising cell.  Initial conditions: Power supply providing 26V and battery is ON.
    This test requires user action to raise and lower temperature"""
    ...
    def setUpClass(cls):
        ...
        cls.tmin = 0   # 'cell under-temperature charging disable'
        ...
        cls.test_fixture.logger.filename = 'rising_temperature_test'
        cls.test_fixture.logger.start()

        print("Apply heat to one battery temperature sensor.")
        # Use a heat gun to slowly raise the temperature of a battery sensor

        while cls.test_fixture.can_monitor.max_temperature <= cls.tmax:
            time.sleep(1)
        
        time.sleep(10) # time hysteresis
        cls.charge_enable_at_tmax = cls.test_fixture.bin_monitor.chargeEnable
        cls.contactor_at_tmax = cls.test_fixture.bin_monitor.contactor
        cls.ccl_at_tmax = cls.test_fixture.can_monitor.charge_current_limit

        print("Remove heat from the battery temperature sensor.")
        while cls.test_fixture.can_monitor.max_temperature > cls.tmax - 2:
            time.sleep(1)
        
        time.sleep(10) # for time hysteresis plus precharge

        cls.charge_enable_at_tmax_less_2c = cls.test_fixture.bin_monitor.chargeEnable
        cls.contactor_at_tmax_less_2c = cls.test_fixture.bin_monitor.contactor
        cls.ccl_at_tmax_less_2c = cls.test_fixture.can_monitor.charge_current_limit
    ...
    def test_charge_enable_state_at_tmax(self):
        self.assertEqual(
            self.charge_enable_at_tmax, 
            0, 
            msg="Charge Enable State Mismatch: Charge Enable = " + str(self.charge_enable_at_tmax) + ', Should be 0')
    
    def test_contactor_state_at_tmax(self):
        self.assertEqual(
            self.contactor_at_tmax, 
            0, 
            msg="Contactor State Mismatch: Contactor = " + str(self.contactor_at_tmax) + ', Should be 0')
    
    def test_charge_current_limit_at_tmax(self):
        self.assertEqual(
            self.ccl_at_tmax, 
            0, 
            msg="charge Current Limit Value Mismatch: CCL = " + str(self.ccl_at_tmax) + ', Should be 0')
    
    def test_charge_enable_state_at_tmax_less_2c(self):
        self.assertEqual(
            self.charge_enable_at_tmax_less_2c, 
            1, 
            msg="Charge Enable State Mismatch: Charge Enable = " + str(self.charge_enable_at_tmax_less_2c) + ', Should be 1')
    
    def test_contactor_state_at_tmax_less_2c(self):
        self.assertEqual(
            self.contactor_at_tmax_less_2c, 
            1, 
            msg="Contactor State Mismatch: Contactor = " + str(self.contactor_at_tmax_less_2c) + ', Should be 1')
    
    # This may not be testable.
    def test_charge_current_limit_at_tmax_less_2c(self):
        self.assertGreater(
            self.ccl_at_tmax_less_2c, 
            0, 
            msg="Charge Current Limit Value Mismatch: CCL = " + str(self.ccl_at_tmax_less_2c) + ', Should be > 0')
